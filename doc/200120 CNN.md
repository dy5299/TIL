...
예전에는 사람이 어느정도 걸러낸 이미지만 사용하고 (ex. 고양이만 있는 영상), gray image 주로 사용했다. (데이터 용량때문에)

요즘은 배경 많은 것도 사용한다. 이는 알아서 걸러내어 학습해라는 의미이다.



older ~: data 많다고 학습 능력이 더 올라가는 것이 아니고, 성능 개선에는 한계가 있다.고 생각함

deep learning: amout of data 많아질수록 성능 개선 효과 있다.



고정밀도 실수 연산은 필요 없다. 결국 weight value 구하는 것인데.. weight value 좀  잘못된 게 있어도 큰 영향 없다.
...
sw공학에서.. core는 늘릴 수 있지만 cpu 클럭수는 더이상 못 늘어난다. 이미 한계가 어느정도 왔다. core늘려도 체감속도 그다지 빨라지지 않는다.



clear-dataset : variation이 더 심함.

augmentation 해도 30%.. 거의 random probability
...
max_pooling으로 줄이는 건 high-level feature 위해서니까

몇 레벨정도까지는 training data에 맞게 학습

밑에 몇 레벨만 프리징하면 2~3% 성능 향상되더라



데이터 전처리 - 정규화하면 개선될 여지 O.

일반적으로 다 함. 학습을 좀 더 빨리 할 수 있는 방법.

정규화(to 0~1)는 dimension 별로 한다.

scsaling 하려고, 값 범위를 보기 위해 항상 print해야 해. (헷갈리지 않기 위해)

정규화: data를 maximum으로 나눠 0~1 범위가 되게 scaling