...
ex) phone gateway - 구글 AI 듀플렉스 : 전화를 대신 걸어주는 AI

### 인공지능 스피커와 챗봇

스마트폰은 특화된 서비스 연동을 목적으로 하는 반면, 인공지능 스피커는 스마트폰에 있는 인공지능엔진과 똑같은데 음성입력을 STT, 음성출력을 TTS로 하여 보편적인 서비스를 연동하는게 목적이다. (개인보다는 가정 타겟)

#### 챗봇 적용 분야 예측

긴급한 상황에서 빠른 답변을 필요로 하는 상황에서, resloving a complaint or problem, getting detailed explanations...

#### 챗봇 사용 이유

- 고객편의, 비용절감, 매출증대
- 비즈니스 환경의 변화 - 모바일 메신저의 활성화
- 자연어 처리 기술의 발전      ex) 실시간 번역
- 관련 eco-system 발전
  - API 플랫폼의 발전
